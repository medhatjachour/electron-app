  ...
  const handlePrint = async () => {
    ...
    try {
      ...
      const receiptData = {
        ...
        paymentMethod: transaction.paymentMethod === 'cash' ? 'نقدي' : 
                       transaction.paymentMethod === 'card' ? 'بطاقة' : 
                       transaction.paymentMethod === 'installment' ? 'تقسيط' : 
                       'آخر',

        // Customer
        customerName: transaction.Customer?.name || 'زبون نقدي',
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-slate-50 dark:bg-slate-900">
          ...
          >
            ...
            <div className="text-center mb-4">
              ...
              <p className="text-xs">ت: {settings.storePhone}</p>
            ...
            <div className="border-t border-b border-gray-300 py-2 mb-3 text-xs">
              ...
              {settings.commercialRegister && <p>س.ت: {settings.commercialRegister}</p>}
            </div>

            {/* Receipt Details */}
            <div className="mb-3 text-xs space-y-1">
              <p>رقم الفاتورة: {transaction.id.substring(0, 8).toUpperCase()}</p>
              <p>التاريخ: {new Date(transaction.createdAt).toLocaleString('ar-EG', {
                year: 'numeric',
                month: '2-digit',
                day: '2-digit',
                hour: '2-digit',
              ...
              {transaction.Customer && <p>العميل: {transaction.Customer.name}</p>}
            </div>
            ...
            <div className="text-xs space-y-1 mb-3">
              ...
              <div className="flex justify-between">
                ...
                <span>{transaction.tax.toFixed(2)} ج.م</span>
              </div>
